...
void Flash_ReadRecordInfo(uint32_t *address, uint16_t *count)
{
    ...
    // 从Flash的固定位置读取记录信息（地址0x000000）
    Flash_ReadData(info_data, 0x000000, 6);
...
void Flash_SaveRecordInfo(uint32_t address, uint16_t count)
{
    ...
    // 准备数据
    info_data[0] = (address >> 24) & 0xFF;
    info_data[1] = (address >> 16) & 0xFF;
    info_data[2] = (address >> 8) & 0xFF;
    info_data[3] = address & 0xFF;
    info_data[4] = (count >> 8) & 0xFF;
    info_data[5] = count & 0xFF;
    
    // 保存到Flash的固定位置（地址0x000000）
    Flash_WritePage(info_data, 0x000000, 6);
}
...
void SaveTemperatureData(int16_t raw_temp)
{
    ...
    // 防止地址溢出（保留前6字节用于存储记录信息）
    if(flash_address >= 0x1FFFE)
    {
        flash_address = 0x000006;  // 从第6字节开始，避免覆盖记录信息
        record_count = 0;
        UART_SendString("Flash address reset to 0x000006\r\n");
        Flash_SaveRecordInfo(flash_address, record_count);
    }
}
...
void DisplayAllRecords(void)
{
    ...
    int16_t raw_temp;
    float temp_c;
    uint32_t address = 0x000006;  // 从第6字节开始读取数据
    uint16_t i;
    ...
    for(i = 0; i < record_count; i++)
    {
        ...
        UART_SendByte('0' + (int)(temp_c * 10) / 100);
        UART_SendByte('0' + ((int)(temp_c * 10) / 10) % 10);
        UART_SendByte('.');
        UART_SendByte('0' + (int)(temp_c * 10) % 10);
        UART_SendString("C\r\n");
...
{   
    ...
    LPC_TMR32B0->PR = 9;
    LPC_TMR32B0->MCR = 3;
    LPC_TMR32B0->MR0 = SystemCoreClock / 10;  // 0.1秒中断
...
{  
    if(LPC_TMR32B0->IR & 0x01)
    {
        ...
        if(LPC_UART->LSR & (1<<0))
        {
            ...
            if((command == 's' || command == 'S') && !recording_enabled)
            {
                recording_enabled = 1;
        ...
        int display_value = (int)(current_temp * 10);
        if(display_value < 0) display_value = -display_value;
        
        UART_SendByte('0' + display_value / 100);
...
int main(void)
{
    ...
    // 检查读取的记录信息是否有效
    if(flash_address > 0x1FFFE || record_count > 0x1FFF)
    {
        // 无效数据，重新初始化
        UART_SendString("Invalid flash data, erasing...\r\n");
        Flash_EraseChip();
        flash_address = 0x000006;  // 从第6字节开始存储数据
        record_count = 0;
        Flash_SaveRecordInfo(flash_address, record_count);
    }
    else
    {
        UART_SendString("Resumed from previous session\r\n");
