...
void SaveTemperatureData(int16_t raw_temp)
{
    ...
    {
        flash_address = DATA_START_ADDRESS;  // 回到数据起始地址
        record_count = 0;
        UART_SendString("Flash address reset to DATA_START_ADDRESS\r\n");
        Flash_SaveRecordInfo(flash_address, record_count);
    }
}
...
void DisplayAllRecords(void)
{
    ...
    int16_t raw_temp;
    float temp_c;
    uint32_t address = DATA_START_ADDRESS;  // 从数据起始地址开始读取
    uint16_t i;
...
int main(void)
{
    ...
    // 显示欢迎信息
    UART_SendString("\r\nTemperature Monitor with Magic Number Validation\r\n");
