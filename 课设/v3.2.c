...
void DisplayAllRecords(void)
{
    ...
    for(i = 0; i < record_count; i++)
    {
        ...
        // 显示三位小数
        int integer_part = (int)temp_c;
        int decimal_part = (int)((temp_c - integer_part) * 1000);
        if(decimal_part < 0) decimal_part = -decimal_part;
        
        UART_SendByte('0' + integer_part);
        UART_SendByte('.');
        UART_SendByte('0' + decimal_part / 100);
        UART_SendByte('0' + (decimal_part / 10) % 10);
        UART_SendByte('0' + decimal_part % 10);
        UART_SendString("C\r\n");
...
{  
    if(LPC_TMR32B0->IR & 0x01)
    {
        ...
        // 显示三位小数
        int integer_part = (int)current_temp;
        int decimal_part = (int)((current_temp - integer_part) * 1000);
        if(decimal_part < 0) decimal_part = -decimal_part;
