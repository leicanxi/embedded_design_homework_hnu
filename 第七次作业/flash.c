...
void DisplayAllRecords(void)
{
    ...
    for(i = 0; i < record_count; i++)
    {
        ...
        UART_SendString("C (0x");
        
        // 显示原始数据
        //UART_SendByte((raw_temp >> 12) & 0xF < 10 ? '0' + ((raw_temp >> 12) & 0xF) : 'A' + ((raw_temp >> 12) & 0xF) - 10);
        //UART_SendByte((raw_temp >> 8) & 0xF < 10 ? '0' + ((raw_temp >> 8) & 0xF) : 'A' + ((raw_temp >> 8) & 0xF) - 10);
       // UART_SendByte((raw_temp >> 4) & 0xF < 10 ? '0' + ((raw_temp >> 4) & 0xF) : 'A' + ((raw_temp >> 4) & 0xF) - 10);
        //UART_SendByte(raw_temp & 0xF < 10 ? '0' + (raw_temp & 0xF) : 'A' + (raw_temp & 0xF) - 10);
       // UART_SendString(")\r\n");
...
int main(void)
{
    ...
    // 显示欢迎信息
    UART_SendString("\r\nTemperature Monitor\r\n");
    UART_SendString("Commands: s=start, t=stop, r=show records\r\n");
